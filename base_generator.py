...
def base_generator(roots_sequence, num_steps=16, temperature=0.5, qpm=120, chords_per_bar=1):
    ...
    for n in roots_sequence.notes:
        for i in range(chords_per_bar):
            root_melody = Melody(ast.literal_eval("[" + str(n.pitch - 24) +"]"))
    ...
    durations = [4*60/qpm] * len(roots_sequence.notes)
    base_sequence = concatenate_sequences(base_sequences_list, durations)
